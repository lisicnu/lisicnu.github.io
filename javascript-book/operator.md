...
-   `a++` 理解为：先把 a 的值赋值给表达式，然后 a 再自增。

-   `++a` 理解为：a 先自增，然后把自增后的值赋值给表达式。
...
> 问题：为什么 NaN, null, undefined 的值是如此？
...
（2）正号不会对数字产生任何影响。比如说，`2`和`+2`是一样的。

（3）我们可以对其他的数据类型使用`+`，来将其转换为 number【重要的小技巧】。比如：
...
逗号运算符也是一个运算符, 所以也有运算符结果。它的运算符结果是**最后一个表达式**的结果。
...
> 备注：在实际写代码的时候，不同优先级符号嵌套时 最好加上括号。
